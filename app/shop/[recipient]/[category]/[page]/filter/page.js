  ...
  useEffect(() => {
    if(filterType === "add"){
      if(props.params.category === "search" && !filters.includes("prices")){
        if(props.searchParams.prices){
          if(props.searchParams.prices === "Up to $25"){
            if(filterView.length === 0 ){
              ...
              dispatch((filterOn(filterView.filter( result => result.price <= 25))))
            }
          }
          if(props.searchParams.prices === "$25-$50") {
            if(filterView.length === 0 ){
              ...
              dispatch((filterOn(filterView.filter( result => result.price >= 25 && result.price <= 50))))
            }
          }
          if(props.searchParams.prices === "$50-4100") {
            if(filterView.length === 0 ){
              ...
              dispatch((filterOn(filterView.filter( result => result.price >= 50 && result.price <= 100))))
            }
          }
          if(props.searchParams.prices === "$100-$200") {
            if(filterView.length === 0 ){
              ...
              dispatch((filterOn(filterView.filter( result => result.price >= 100 && result.price <= 200))))
            }
          }
          if(props.searchParams.prices === "$200+") {
            if(filterView.length === 0 ){
              ...
              dispatch((filterOn(filterView.filter( result => result.price >= 200))))
            }
          }
          if(filterType === "add"){
        ...
        if(props.searchParams.prices && !filters.includes("prices")){ // get rid of subtract, make it itsown if statement at bottom of if// add another
          if(props.searchParams.prices === "Up to $25"){
            if(filterView.length === 0 ){
      ...
      for(let keys in checklist){
          if(checklist[keys] === "Up to $25"){
            ...
            baseList = baseList.filter( result => result.rating >= 4 && result.rating < 5 )
          } else if(checklist[keys] === "3 Stars"){
            baseList = baseList.filter( result => result.rating >= 3  && result.rating < 4)
          } else if(checklist[keys] === "2 Stars"){
            baseList = baseList.filter( result => result.rating >= 2 && result.rating < 3)
          } else if(checklist[keys] === "1 Star"){
            baseList = baseList.filter( result => result.rating >= 1 && result.rating < 2)
          } else if(checklist[keys] === "<10"){
            baseList = baseList.filter( result => result.reviews < 10)
          } else if(checklist[keys] === "10-100"){
            baseList = baseList.filter( result => result.reviews >= 10 && result.reviews <= 100)
          } else if(checklist[keys] === "100-250"){
            baseList = baseList.filter( result => result.reviews >= 100 && result.reviews <= 250)
    ...
    const clickHandler = (event) => {
      ...
      if(event.target.id === "back" && Number(props.params.page) > 1) {
        if(props.params.category === "search"){
            path = `/shop/${props.params.recipient}/search/${Number(props.params.page) - 1}/filter?`
            path += filters
            router.push(path)
        } else {
